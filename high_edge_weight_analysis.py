...
plt.savefig('Viral_Genomics/outputs/histogram_cluster_counts_300.png')

#%%
# Analysis pre-modern high edge weights

percentile_95 = np.percentile(pre['edge_weight'], 95)

# Filter  based on the selected 90 percentile cut off
high_pre = pre[pre['edge_weight'] >= percentile_95]
...
#%%
all_connections = pd.concat([pre, non, ind])
all_connections_with_label = pd.merge(all_connections, network_labels_df, left_on='Source', right_on='Genome', how='left')

#%%
all_percentile_90 = np.percentile(all_connections_with_label['edge_weight'], 90)
